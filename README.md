...
* Files with `.createElement+render.js` suffix.
* Files with `.spec.js` suffix.

The `.createElement+render.js` / `.spec.js` files (or the `__tests__` folders) can be located at any depth under the `src` top level folder.

We recommend to put the test files (or `__tests__` folders) next to the code they are testing so that relative imports appear shorter. For example, if `App.createElement+render.js` and `App.js` are in the same folder, the test just needs to `import App from './App'` instead of a long relative path. Colocation also helps find tests more quickly in larger projects.
...
This test mounts a component and makes sure that it didn’t throw during rendering. Tests like this provide a lot of value with very little effort so they are great as a starting point, and this is the test you will find in `src/App.createElement+render.js`.
...
in all web browsers. Service worker registration [won't be attempted](src/history/registerServiceWorker.js)
...
starting point, you can make use of the logic included in [`src/registerServiceWorker.js`](src/history/registerServiceWorker.js), which
...
**react+less+ant环境配置**
> -  less 要将javascript 支持 
> -  如果react脚手架支持less用法，则要在引入antd.css的时候，改成引入antd.less
